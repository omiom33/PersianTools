...
# ----------------------------------------Wrong dictations-----------------------
forReplace2 = {u'فعالیتها': u'فعالیت‌ها'
}
...
def quotation(text):
    ...
    for i in range(0,len(lines)):
        ...
        line = re.sub(ur'(^|[' + persianCharacters+ ur'\:>،»؛ \n\}\]\.]+)“((?:\[\[|).*?['+ persianCharacters+ ur'\n]+?(?:\]\]|\.|\<|\:|))”(['+ persianCharacters+ ur'،«؛ \n\.\[\{]|$)', u'\\1«\\2»\\3',line)
        # وارونه ”“ تبدیل
...
def dictationReplace(x,y,extensions,text):
    Regex=ur'(^|[^' + persianCharacters + ur'])( |\u200c|_|)(' + x + ur')( |_)('+ y + ur')( |\u200c|_|)(' + extensions + ur')(\n|[^' + persianCharacters + ur'])'
...
def dictation(text):
    ...
    for i in forReplace2:
        if forReplace2.has_key(i):
            text = re.sub(ur'(^|[^' + persianCharacters
                                + ur'])( |\u200c|_|)(' + i
                                + ur')( |\u200c|_|)($|[^'
                                + persianCharacters + ur'])', u'\\1\\2'
                                + forReplace2[i] + u'\\4\\5',text)
    # کلماتی که آ دارند

    text = complex_replace(text, ur"(^| |_|«|»|\[|\(|\<|\>|\')("+ wordsWithA+ ur")(ی|ئی|یی|ٔ|)( |«|»|\.|،|_|\]| |\:|\)|\<|\>|؟|\'|\!|$)", u'\u0627', ur'آ')
    ...
    text = complex_replace(text, ur"(^| |_|«|»|\[|\(|\<|\>|\')("
                           + HamzehZam
                           + ur")(‌ها|ها|ین|ان|ی|ئی|یی|ٔ|)( |«|»|\.|،|_| |\]|\:|\)|\<|\>|؟|\!|\'|$)"
                           , u'\u0648', ur'ؤ')
    ...
    for i in HamzehAtInside:
        if HamzehAtInside.has_key(i):
            ...
            # الف مقصوره
            text = re.sub(ur"(^| |_|«|»|\[|\(|\<|\>|\')(" + HamzehAtEnd + ur")(?= |«|»|\.|،|_| |\]|\:|\)|\<|\>|؟|\!|\'|$)",u'\\1\\2ء',text)
...
def applyOrthography(text):
    ...
    text = re.sub(ur'([' + persianCharacters + vowels + hamza+ ur'])( )([' + vowels + hamza + ur'])',u'\\1\\3\\2',text)
...
def applyZwnj(text):
    ...
    # ای «توان» ناقلا!
    '''
    text = re.sub(ur"( )(می|نمی) ?توان", u'\\1\\2\u200cتوان',text)

    # چسباندن تمام «ها»ها با فاصلهٔ مجازی

    text = re.sub(ur" ها([\]\.،\:»\) ]|\'{2,3}|\={2,})",u'\u200cها\\1',text)
    text = re.sub(ur" ها(ی|یی|یم|یت|یش|مان|تان|شان)([\]\.،\:»\) ])", u'\u200cها\\1\\2',text)
    text = re.sub(ur"هها", u'ه‌ها',text)

    # چسباندن تمام «ترین»ها با فاصلهٔ مجازی

    text = re.sub(ur" ترین([\]\.،\:»\) ]|\'{2,3}|\={2,})",u'\u200cترین\\1',text)
...
def SimpleTextPunctuation(text):
    #عدد منتهی به کلمه، بین حروف و اعداد فاصله بگذارید.
    text = re.sub(ur"([\d۱۲۳۴۵۶۷۸۹۰]+)([\u0621-\u0655\u067E\u0686\u0698\u06AF\u06A9\u0643\u06AA\uFED9\uFEDA\u06CC\uFEF1\uFEF2]+)", u'\\1 \\2',text)
    #کلمه منتهی به رقم، بین حروف و اعداد فاصله بگذارید.
    text = re.sub(ur"([\u0621-\u0655\u067E\u0686\u0698\u06AF\u06A9\u0643\u06AA\uFED9\uFEDA\u06CC\uFEF1\uFEF2]+)([\d۱۲۳۴۵۶۷۸۹۰]+)", u'\\1 \\2',text)
    #کلمه انگلیسی منتهی به الفبای فارسی، بین کلمات فارسی و انگلیسی فاصله بگذارید.
    text = re.sub(ur"([a-zA-Z]+)([\u0621-\u0655\u067E\u0686\u0698\u06AF\u06A9\u0643\u06AA\uFED9\uFEDA\u06CC\uFEF1\uFEF2]+)", u'\\1 \\2',text)
    #کلمه فارسی منتهی به الفبای انگلیسی، بین کلمات فارسی و انگلیسی فاصله بگذارید.
    text = re.sub(ur"([\u0621-\u0655\u067E\u0686\u0698\u06AF\u06A9\u0643\u06AA\uFED9\uFEDA\u06CC\uFEF1\uFEF2]+)([a-zA-Z]+)", u'\\1 \\2',text)
    #فاصله قبل از شماره پاورقی، فاصله قبل از شماره پاورقی را حذف کنید.
    text = re.sub(ur"([\u0621-\u0655\u067E\u0686\u0698\u06AF\u06A9\u0643\u06AA\uFED9\uFEDA\u06CC\uFEF1\uFEF2])[ ]+([\u0002])", u'\\1\\2',text)
    #پاورقی در جای نادرست
    text = re.sub(ur"([\)\»\.])[ ]*([\u0002])(.)", u'\\2\\1\\3',text)
    #فاصله بعد از پاورقی لازم است. بعد از پاورقی فاصله بگذارید.
    text = re.sub(ur"([\u0621-\u0655\u067E\u0686\u0698\u06AF\u06A9\u0643\u06AA\uFED9\uFEDA\u06CC\uFEF1\uFEF2])([\u0002])([^ \.!؟\؛،\)\]\»])", u'\\1\\2 \\3',text)
    #فاصله قبل از فرمول لازم است. قبل از فرمول فاصله بگذارید.
    text = re.sub(ur"([^\(\)\»\« ])([\u0001])", u'\\1 \\2',text)
    #فاصله بعد از فرمول لازم است. بعد از پاورقی فاصله بگذارید.
    text = re.sub(ur"[ ]([\u0001])([^\(\)\»\« ،\؛;,!؟\?!])", u' \\1 \\2',text)
    #---- تغییرجهت دهنده متن ---
    #افزودن تغییرجهت دهنده متن برای سجاوندی‌های بعد از حروف لاتین
    #text = re.sub(ur"([A-Za-z])([\+\-\*\/\\\.\?!#$%\^&\(\),:;~\=_]+) ([^A-Za-z0-9])", u'\\1\\2\202B \\3',text)
    #افزودن تغییرجهت دهنده متن برای سجاوندی‌های بعد از حروف لاتین
    #text = re.sub(ur"([^A-Za-z0-9]) ([\+\-\*\/\\\.\?!#$%\^&\(\),:;~\=_]+)([A-Za-z])", u'\\1 \u200E\\2\\3',text)
    return text
...
def punctuation(text):
    ...
    #text = re.sub(ur"(. ) +(?=[^= ])", u'\\1',text) #در پاورپوینت خرابکاری می‌کند

    # فاصله بعد از سجاوندی به جز ! به دلیل (<!-- و !! در بالای جدول‌ها)

    text = re.sub(ur"([،\.\؛\؟»\!;,])([^ \.\(\)«»\"\[\]<>\d\w\{\}\|۰۱۲۳۴۵۶۷۸۹\'])", u'\\1 \\2',text)
...
def run (text):
    #if text:
    if re.sub(ur"[a-zA-Z]+ *[\)\[\]\{\}\:]",u'',text)==text:#This has direction problem with lathin words in prantesis
        text = normalizeZwnj (text)
        text = quotation (text)
        text = dictation (text)
        ...
        text = SimpleTextPunctuation (text)
        text = normalizeZwnj (text)
        text = re.sub(ur"([A-Za-z])([\+\-\*\/\\\.\?!#$%\^&\(\),:;~\=_]+|) ([^A-Za-z0-9])", u'\\1\\2\u202B \\3',text)
        text = re.sub(ur"([^A-Za-z0-9]) ([\+\-\*\/\\\.\?!#$%\^&\(\),:;~\=_]+|)([A-Za-z])", u'\\1 \u202A\\2\\3',text)
    return text
...
def Open_pptx(file_path):
    ...
    for Slide in Presentation.Slides:
        ...
        for Shape in Slide.Shapes:
            ...
            Text_num+=1
            print Text_num
            text=Out_texts[Text_num]
            text2=Shape.TextFrame.TextRange.Text
            #print (repr(text2))
            #text=text.replace(u'\v',u'\r\n').replace(u'\r\r',u'\r')
            if re.sub(u'['+persianCharacters+u']',u'',text)!=text:
                my_result=run (text)
                if text!=my_result:
                    txt_edited_num+=1
                    Shape.TextFrame.TextRange.Text=my_result.replace(u'\n',u'\r\n').replace(u'\r\r',u'\r')
    ...
    print 'Finished at '+strftime("%Y-%m-%d %H:%M:%S", gmtime())+ 'at '+str(minute) +' Minutes'
...
def do_test():
    ...
    test (35,u'متن '+ u'متن۱۲۵ متنabc'+u' متن',u'متن '+ u'متن ۱۲۵ متن abc'+u' متن')
    test (36,u'متن '+ u'ا ۱۲۵متن abcمتن'+u' متن',u'متن '+ u'ا ۱۲۵ متن abc متن'+u' متن')
    test (37,u'متن '+ u'متن . نقطه'+u' متن',u'متن '+ u'متن. نقطه'+u' متن')

# This tool edits the powerpoint file
file_path=u'c:/example.pptx'
Open_pptx(file_path)
